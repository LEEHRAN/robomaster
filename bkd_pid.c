...
void PID_SET_Init(void)
{
	...
	PID_Init(&Yaw_speed_pid,180,60,2,0,30000,0,0,99999999);//Yaw和Pitch的速度环和位置环  0.85 2.4
	PID_Init(&Yaw_position_pid,15,2,0,0,16000,0,0,999999);// 8
	PID_Init(&Yaw_Auto_Aim_pid,1.1,0.2,0.05,0,36000,0,0,7000);
	
	PID_Init(&Pitch_speed_pid,-37,-3,0,0,30000,0,0,36000);   
	PID_Init(&Pitch_position_pid,0.75,0.2,0,0,16000,0,0,36000);   

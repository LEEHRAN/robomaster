...
void Cloud(void)
{
	...
	PID_Calc_Position(&Pitch_position_pid,pitchposition*0.2,0);
	PID_Calc_Position(&Pitch_speed_pid,(double)gx/32.8f*1.8,Pitch_position_pid.output);//Pitch_position_pid.output
	if(((Yaw_position_Feedback.real[0]-YAW_Mechanics_Angle)<50)&&((Yaw_position_Feedback.real[0]-YAW_Mechanics_Angle)>-50+6))
		Cloud_Init_Flag=1;
	}
